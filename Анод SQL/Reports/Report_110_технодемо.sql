     /*оставлен как пример поиска компонентов и изделий под ПВ, не рабочие моменты закомментированы*/
     ...
     WITH
          ...
          tech_op_list AS (
             SELECT accepted_list.id AS id_accepted_list,
                    ...
                    (
                       SELECT ARRAY_AGG(DISTINCT comp_task.id)
                         FROM registry.object_2094_ comp_task
                    LEFT JOIN registry.object_1409_ comp_task_link_comp ON comp_task_link_comp.id = comp_task.attr_2100_
                          AND comp_task_link_comp.is_deleted IS FALSE
                        WHERE comp_task.attr_2173_ = accepted_list.attr_2226_
                          AND comp_task_link_comp.attr_1458_ = accepted_list.attr_2632_
                          AND comp_task.is_deleted IS FALSE
                    ) array_comp_task,
                    accepted_list.attr_2632_ nomenclature,
                    accepted_list.attr_2167_ id_units_materials,
                    (
                       SELECT ARRAY_AGG(DISTINCT comp_task_link_comp.id)
                         FROM registry.object_1409_ comp_task_link_comp
                    LEFT JOIN registry.object_2094_ comp_task ON comp_task_link_comp.id = comp_task.attr_2100_
                          AND comp_task.is_deleted IS FALSE
                    LEFT JOIN registry.object_2093_ task ON task.id = comp_task.attr_2173_
                          AND task.is_deleted IS FALSE
                        WHERE comp_task_link_comp.attr_1650_::int = ANY (accepted_list.attr_2675_)
                          AND comp_task_link_comp.attr_1458_ = ANY (accepted_list.attr_3264_)
                          AND comp_task.id IS NOT NULL
                          AND task.attr_3206_ IS TRUE
                          AND comp_task_link_comp.is_deleted IS FALSE
                    ) array_izd,
                    SUM(
                    CASE
          ...
          dataset AS (
             SELECT orders.id AS orders_id,
                    ...
                    CASE
                              WHEN comp_position_orders.attr_1411_ = 82 THEN CONCAT_WS(' ', /*mat.attr_401_*/ 'материал')
                              ELSE CONCAT_WS(
                              ' ',
                              reg_mat.attr_2976_,
                              reg_sort.attr_401_,
                    ...
                    CASE
                              ...
                              WHEN comp_position_orders.attr_1411_ = 82 THEN /*units__mat.attr_390_*/'ед. изм.'
                              ELSE CASE
                    ...
                    CASE
                              ...
                              WHEN comp_position_orders.attr_1411_ = 82 THEN 0 /*units__mat.attr_390_*/
                    ...
                    CASE
                              ...
                              ELSE CASE
                                        ...
                                        COALESCE(/*nom_tech_card.attr_4105_*/0, 0) * CEILING(
                                        ...
                                        WHEN comp_position_orders.attr_1411_ = 82 THEN comp_position_orders.attr_1896_ * 0  /*units__mat.attr_390_*/
                              END
          ...
          LEFT JOIN registry.object_301_ nomenclature ON comp_position_orders.attr_1458_ = nomenclature.id
          LEFT JOIN registry.object_389_ units_nomenclature ON nomenclature.attr_379_ = units_nomenclature.id
          ...
          LEFT JOIN registry.object_519_ prod_tech_card ON (
                    CASE
                              WHEN comp_position_orders.attr_1463_ = 2
                                     OR comp_position_orders.attr_1421_ IN (6, 4) THEN prod_tech_card.attr_1466_
                                        ELSE prod_tech_card.attr_520_
                    END = CASE
                              WHEN comp_position_orders.attr_1463_ = 2
                                     OR comp_position_orders.attr_1421_ IN (6, 4) THEN comp_position_orders.ID
                                        ELSE nomenclature.ID
                    END
                    ) /*выбираем карту из компонента, если его изменили или создали не из ном. ед.*/
                AND prod_tech_card.is_deleted IS FALSE
                AND prod_tech_card.attr_2908_ IS TRUE /*активная карта*/
          ...
          LEFT JOIN tech_op_list ON work_task_comp.id = ANY (tech_op_list.array_comp_task)
                 OR (
                    tech_op_list.nomenclature = comp_position_orders.attr_1458_
