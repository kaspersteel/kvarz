   SELECT o.id
        ...
        , CASE
                    WHEN comp_pos_ord.attr_1421_ = 9 THEN CONCAT(
                    'По заказу № '
                  , ord.attr_607_
...
LEFT JOIN registry.object_301_ sprav_nom ON o.attr_2632_ = sprav_nom.id
...
LEFT JOIN registry.object_2094_ comp_task ON o.attr_2632_ = comp_task.attr_3203_
      AND comp_task.attr_2101_::INTEGER = ANY (o.attr_4033_)
          /*выбор компонентов заказа по ссылке из компонента задания*/
LEFT JOIN registry.object_1409_ comp_pos_ord ON comp_task.attr_2100_ = comp_pos_ord.id
LEFT JOIN registry.object_400_ sprav_mat ON comp_task.attr_3902_ = sprav_mat.id
LEFT JOIN registry.object_3168_ pos_task ON comp_task.attr_3175_ = pos_task.id
LEFT JOIN registry.object_1227_ pos_ord ON comp_pos_ord.attr_1423_ = pos_ord.id
LEFT JOIN registry.object_606_ ord ON pos_ord.attr_1923_ = ord.id
    WHERE o.is_deleted IS FALSE
      AND o.attr_3193_ = pos_task.id
 GROUP BY o.id
        , t_op.attr_2609_
        , t_op.attr_2152_
        , t_op.attr_3208_
