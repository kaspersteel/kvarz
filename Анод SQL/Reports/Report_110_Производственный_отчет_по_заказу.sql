     WITH
        ...
        , dataset AS (
          ...
          В руб.*/
             SELECT orders.id orders_id
                  ...
                  , CASE
                    ...
                    END type_ne
                    /*Выбор далее в зависимости от того, вносились ли изменения в компонент (attr_1421_)*/
          ...
          LEFT JOIN tech_op_list ON tech_op_list.nom_units = comp_position_orders.attr_1482_
                AND (
                    work_task_comp.id = ANY (tech_op_list.array_comp_task)
                 OR (
                    comp_position_orders.attr_1414_::INT = ANY (tech_op_list.array_izd)
          ...
          /*таблица данных с распределением оприходованного по ПВ на компоненты, изготавливаемые по этой ПВ*/
   ...
   SELECT division.*
        , CASE
                    WHEN division.source <> 2 THEN 'не произв.'
