     WITH dataset AS (
             SELECT requirement_invoice.id AS id_requirement_invoice
                  ...
                  , (
                       SELECT COALESCE(remnants.attr_2574_, 16)
                         FROM registry.object_1617_ remnants
                        WHERE remnants.id = (
                                 SELECT remnants_max.id
                                  ...
                                  WHERE remnants_max.nom_id = table_requirement_invoice.attr_2115_
                                    AND remnants_max.given = table_requirement_invoice.attr_4121_
          ...
          LEFT JOIN registry.object_400_ tr ON table_requirement_invoice.attr_2118_ = tr.id
              ...
              WHERE requirement_invoice.is_deleted IS FALSE
          )
   SELECT dataset.*
...
LEFT JOIN registry.object_1685_ sprav_storage ON dataset.storage = sprav_storage.id
    ...
    WHERE dataset.id_requirement_invoice = '{superid}'
      AND CASE
                    WHEN '{type}' = 1 AND dataset.storage NOT IN (14, 15, 16) THEN TRUE
                    WHEN '{type}' = 2 AND dataset.storage NOT IN (4, 5, 6, 9, 12, 14, 15, 16) THEN TRUE
                    WHEN '{type}' = 3 AND dataset.storage = '{storage}' THEN TRUE
          END