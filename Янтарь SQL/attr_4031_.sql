...
WITH proc AS (
		...
		) AS proc ( "name", "interval", "id" )  
		ON o.is_deleted=false /*не трогаем удаленные*/
		WHERE o.is_deleted=false /*не включаем удаленные в итог*/) 
		SELECT DISTINCT
        	proc."record" as record,
			concat (
				CASE WHEN proc."id" < proc2."id" THEN proc."name" ELSE proc2."name" END,
				' || ',
				CASE WHEN proc."id" > proc2."id" THEN proc."name" ELSE proc2."name" END 
