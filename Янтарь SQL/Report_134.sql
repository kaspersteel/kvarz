...
--------------------------------------------------//----------------------------------------------------------
	...
	SELECT
		...
		(
		SELECT ARRAY
			[ ARRAY_AGG ( t_ranges.START ), ARRAY_AGG ( t_ranges.finish ) ] 
		FROM
			(
			SELECT
				timeslots.attr_4754_ AS START,
				timeslots.attr_4753_ AS finish,
				T.ID AS ID 
			FROM
				UNNEST ( P.array_time ) AS T ( ID )
				LEFT JOIN registry.object_4002_ timeslots ON timeslots.ID = T.ID 
			) AS t_ranges 
		) AS array_ranges 
	FROM
		registry.object_4000_ o /*строится таблица соответствия id процедуры в плане и её времени по строке расписания*/
		LEFT JOIN LATERAL (
		VALUES
			( o.attr_4008_, o.attr_4007_ ),
			( o.attr_4010_, o.attr_4009_ ),
			( o.attr_4012_, o.attr_4011_ ),
			( o.attr_4014_, o.attr_4013_ ),
			( o.attr_4016_, o.attr_4015_ ),
	...
	FROM
		(
		SELECT UNNEST
			( array_remove ( ARRAY_AGG ( x.c1 ), NULL ) ) || '-' || UNNEST ( array_remove ( ARRAY_AGG ( x.c2 ), NULL ) ) AS period 
		FROM
			(
			SELECT
			CASE
