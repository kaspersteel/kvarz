...
DECLARE
    ...
    excluded_registry_ids_array INT[] := 
        CASE 
            WHEN excluded_text IS NULL OR TRIM(excluded_text) = '' THEN ARRAY[]::INT[]
            ELSE string_to_array(excluded_text, ',')::INT[]
        END;
    ...
    dyn_sql TEXT;
BEGIN
    RAISE NOTICE 'excluded_text: "%"', excluded_text;
    RAISE NOTICE 'excluded_registry_ids_array: %', excluded_registry_ids_array;
    ...
    LOOP
        ...
        -- Поиск ссылающихся записей (ПРАВИЛЬНАЯ ПРОВЕРКА)
        FOR rec IN
            ...
            WHERE xs.entity_type_id = 'xref_field'
              ...
              -- ПРАВИЛЬНО игнорируем исключенные реестры
              AND (excluded_registry_ids_array IS NULL 
                   OR array_length(excluded_registry_ids_array, 1) = 0
    ...
    FOR rec IN
        ...
        WHERE is_deleted IS NOT TRUE
          -- Не удаляем записи из исключенных реестров
          AND (excluded_registry_ids_array IS NULL 
               OR array_length(excluded_registry_ids_array, 1) = 0
               OR NOT (object_id = ANY(excluded_registry_ids_array)))
