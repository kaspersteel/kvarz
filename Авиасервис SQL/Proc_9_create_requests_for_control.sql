...
DECLARE 
    ...
    old_rems BOOLEAN; -- флаг ввода первичныхх остатков
    ...
    FOR tab_invoice IN (
       SELECT array_agg(tab.id ORDER BY tab.attr_110_, tab.id) AS ids_tab_invoice,
        ...
        WHERE tab.id = ANY(v_mat_rows_ids)
          AND (old_rems is true OR old_rems is not true AND tab.attr_808_ = 2)
     ...
     GROUP BY tab.attr_110_, tab.attr_94_, tab.attr_96_, tab.attr_267_
        ...
        /* Проверка: lh_ids_tab_invoice пуст */
        IF lh_ids_tab_invoice IS NULL OR lh_ids_tab_invoice = 0 THEN
            RAISE EXCEPTION 'lh_ids_tab_invoice пуст для ключа (поставщик-материал-плавка-партия). Ошибка агрегации данных.';
        END IF;
        ...
        WHILE v_from <= lh_ids_tab_invoice LOOP
            ...
            FOREACH x IN ARRAY v_slice LOOP
               INSERT INTO registry.object_124_ (
                      attr_126_, attr_127_, attr_130_, attr_131_, attr_132_, 
                      attr_272_, attr_285_, attr_289_, attr_762_, operation_user_id
               )
               SELECT new_request_id, x,
                      COALESCE(mat_dir.attr_856_, up_mat_dir.attr_856_, 1)::text,
                      samples.n::text,
                      COALESCE(mat_dir.attr_755_, up_mat_dir.attr_755_),
                      i, invoice_id, TRUE, 3, do_user
                 FROM registry.object_30_ smp
            LEFT JOIN registry.object_58_ mat_dir ON mat_dir.id = smp.attr_94_ AND mat_dir.is_deleted is not true
            LEFT JOIN registry.object_58_ up_mat_dir ON up_mat_dir.id = mat_dir.attr_61_ AND up_mat_dir.is_deleted is not true
            LEFT JOIN LATERAL ( 
                ...
                /* Проверка: запись в registry.object_124_ создана */
                GET DIAGNOSTICS v_rows_affected = ROW_COUNT;
                IF v_rows_affected = 0 THEN
                    RAISE EXCEPTION 'Не создана запись в object_124_ для ед. хранения id=%', x;
                END IF;
                ...
                /* Проверка: запись в registry.object_30_ обновлена */
                GET DIAGNOSTICS v_rows_affected = ROW_COUNT;
                IF v_rows_affected = 0 THEN
                    RAISE EXCEPTION 'Не обновлена запись в object_30_ для id=% (возможно, запись удалена или заблокирована)', x;
                END IF;
            ...
            /* номер следующей заявки по тому же ключу */
            npp_request := npp_request + 1; 
            i := 1;
        END LOOP;
        
        /* номер следующей заявки по новому ключу */
        npp_request := npp_request + 1; 
        i := 1;
    END LOOP;
